...
<div class="wrapper container">
    ...
    <form id="register-form" class="form-horizontal" method="post">
        ...
        <div class="row">
            <br><b>NOTE:</b> You should put the same section in rows of all the courses that you are currently registered in.<br><br>
        </div>
        ...
        <div class="table-responsive-sm">
            <table class="table">
                ...
                <?php
                $sql = "SELECT id, name, code, short, batch FROM subjects_ee ORDER BY batch DESC, name ASC";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td>" . $row['code'] . "</td>";
                        echo "<td><input type='text' class=\"form-control\" pattern="[A-Ca-c]{1}" placeholder='Enter section like A or B or C' name=\"sections[$id]\"/>";
                        echo "</tr>";
                    }
                }
                ?>
    ...
    if (isset($_POST['submit'])) {
        ...
        if (isset($_POST['sections'])) {
            ...
            for ($i = 0; $i < sizeof($sectionstmp); $i++) {
                if ($sectionstmp[$i]) {
                    ...
                    if (strlen($sectionstmp[$i]) > 1 || !preg_match("/^[a-c]$/i", $sectionstmp[$i])) {
        ...
        $sql = "INSERT INTO `students` (`id`, `active`, `name`, `batch`, `major`, `email`, `subjects`, `sections`) VALUES (NULL, '0', '$name', '$batch', 'KHI-EE', '$email', '$subjects', '$sections')";
